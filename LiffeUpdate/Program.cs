...
namespace mbdt.LiffeUpdate
{
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            string argument = "download";// (0 < args.Length ? args[0] : "update");
            ...
            {
                if (!LiffeDownload.DownloadDays(startDate, true, debugTraceLevel))
                    Environment.ExitCode = 1;
            }
            else if (Command.Download == command)
            {
                if (!LiffeDownload.DownloadDays(startDate, false, 0))
                    Environment.ExitCode = 1;
            }
            else if (Command.Import == command)
            {
                if (!LiffeImport.DoImport(importPath, debugTraceLevel))
                    Environment.ExitCode = 1;
            }
            if (!LiffeImport.DoCleanup())
                Environment.ExitCode = 1;
