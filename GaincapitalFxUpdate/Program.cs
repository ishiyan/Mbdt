    ...
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (Command.Download == command)
                Trace.TraceInformation("Command: download to [{0}]", downloadPath);
            else if (Command.Import == command)
                Trace.TraceInformation("Command: import from [{0}]", importPath);
            else
                Trace.TraceInformation("Command: update: download to [{0}]", downloadPath);
            ...
            {
                if (!GaincapitalFxDownload.Download(startDate, true, debugTraceLevel))
                    Environment.ExitCode = 1;
            }
            else if (Command.Download == command)
            {
                if (!GaincapitalFxDownload.Download(startDate, false, debugTraceLevel))
                    Environment.ExitCode = 1;
            }
            else if (Command.Import == command)
            {
                if (!GaincapitalFxImport.DoImport(importPath, debugTraceLevel))
                    Environment.ExitCode = 1;
            }
            if (!GaincapitalFxImport.DoCleanup())
                Environment.ExitCode = 1;
