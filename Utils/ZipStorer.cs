...
namespace System.IO.Compression
{
    ...
    public class ZipStorer : IDisposable
    {
        ...
        /// <summary>File access for Open method.</summary>
        private FileAccess access;
        /// <summary>Static CRC32 Table.</summary>
        ...
        /// <returns>A valid ZipStorer object.</returns>
        private static ZipStorer Create(Stream stream, string comment)
        {
        ...
        /// <returns>A valid ZipStorer object.</returns>
        private static ZipStorer Open(Stream stream, FileAccess access)
        {
        ...
        /// <param name="commentary">Comment for stored file.</param>        
        public void AddFile(Compression method, string pathname, string filenameInZip, string commentary)
        {
            if (access == FileAccess.Read)
                throw new InvalidOperationException("Writing is not alowed");

            var stream = new FileStream(pathname, FileMode.Open, FileAccess.Read);
            AddStream(method, filenameInZip, stream, File.GetLastWriteTime(pathname), commentary);
            stream.Close();
        }
        /// <summary>
        ...
        /// <param name="commentary">Comment for stored file.</param>
        private void AddStream(Compression method, string filenameInZip, Stream source, DateTime modTime, string commentary)
        {
            if (access == FileAccess.Read)
                throw new InvalidOperationException("Writing is not alowed");

            // Prepare the fileinfo
            var zfe = new ZipFileEntry {Method = method, EncodeUtf8 = EncodeUtf8, FilenameInZip = NormalizedFilename(filenameInZip), Comment = commentary ?? "", Crc32 = 0, HeaderOffset = (uint)zipFileStream.Position, ModifyTime = modTime};
        ...
        public List<ZipFileEntry> ReadCentralDir()
        {
            ...
            for (int pointer = 0; pointer < centralDirImage.Length; )
            {
                ...
                bool isUtf8 = (BitConverter.ToUInt16(centralDirImage, pointer + 8) & 0x0800) != 0;
                ...
                Encoding encoder = isUtf8 ? Encoding.UTF8 : defaultEncoding;
        ...
        private bool ExtractFile(ZipFileEntry zfe, string filename)
        {
        ...
        private void Store(ref ZipFileEntry zfe, Stream source)
        {
            ...
            Stream outStream = zfe.Method == Compression.Store ? zipFileStream : new DeflateStream(zipFileStream, CompressionMode.Compress, true);
