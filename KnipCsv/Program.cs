...
namespace KnipCsv
{
    static class Program
    {
        ...
        private static void Collect(string sourceFileName)
        {
            using (var sourceFile = new StreamReader(sourceFileName))
            {
                ...
                while (null != (line = sourceFile.ReadLine()))
                {
                    ...
                    string[] splitted = line.Split(Properties.Settings.Default.CsvSplitChar);
                    s = splitted[col].Trim();
        ...
        {
            if (args.Length < 2)
                Console.WriteLine("Arguments: csvColumnNumber(zero-based) dir_or_file_name");
            else
            {
                col = int.Parse(args[0]);
                TraverseTree(args[1], Collect);
            }
        }
    }
}
