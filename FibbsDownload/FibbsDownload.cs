...
namespace mbdt.FibbsDownload
{
    internal static class FibbsDownload
    {
        ...
        private static bool DownloadAllSymbols(DateTime dateTime)
        {
            ...
            {
                downloadable = ComposeDownloadable("exfi_all.zip", dateTime);
                try
                {
                    using (ZipStorer zipStorer = ZipStorer.Create(downloadable, ""))
                    {
                        for (int i = 0; i < list.Count; i++)
                        {
                            ...
                            // It is cumbersome to unzip plain csv files because zips are in non-standard format.
                            // Store zips instead.
        ...
        private static string ComposeUrl(string symbol, DateTime dateTime)
        {
            ...
            return string.Format("http://www.fibbs.nl/user/MarketsNew.cgi/{0}{1}?UserId=9cab8a6366fd6da89b9376f9e7844965&Key=9560c0e87739bf7a8ebf2123888c25dd&Action=GetFile&DownLoadFormaat=Fibbs&DownLoadFile={2}{3}", mdStamp, symbol, mdStamp, symbol);
        }
